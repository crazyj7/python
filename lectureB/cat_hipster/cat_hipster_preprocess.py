...
for dirname in dirnames:
    ...
    # 수정이미지(박스만 crop된 이미지), 수정이미지에서의 랜드마크 정보
    dataset2 = {
        'img2':[],
        'lmk2':[]
    }
    ...
    def resize_img(im):
        ...
        ratio = float(img_size) / max(old_size)     # 변경할사이즈 / (기존 가로,세로 중 큰 길이)
        new_size = tuple([int(x*ratio) for x in old_size])  # 변경할 사이즈 크기. (가로,세로 최대 길이에 맞춤)
        # new_size should be in (width, height) format
        im = cv2.resize(im, (new_size[1], new_size[0]))  # 변경할 크기로 이미지 리사이즈
        delta_w = img_size - new_size[1]    # 남은 가로 길이
        delta_h = img_size - new_size[0]    # 남은 세로 길이
        top, bottom = delta_h // 2, delta_h - (delta_h // 2)    # top, bottom 마진 크기
        left, right = delta_w // 2, delta_w - (delta_w // 2)    # left, right 마진 크기
