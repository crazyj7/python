...
```
v1 (c21)
imageaugument, cnn, dropout, trainset으로 훈련 6개로 kfold. 앙상블 ; 98.040. good
tta 적용시 오히려 떨어짐. bad.
--------------
v2 (c33)
trainset에 데이터셋 추가(dig-mnist.csv)
4 kfold ; 98.340.
tta 사용안함
--------------
v3
batchnormal을 conv layer뒤에 추가.
4 kfold : 98.740
```
...
from keras.layers import Conv2D, Dropout, MaxPool2D, Flatten, Dense, Softmax, BatchNormalization
...
class EpochLogWrite(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        tmx = utc.localize(datetime.utcnow()).astimezone(KST).time()
        dbgprint('Epoch #{} ends at {}  acc={} val_acc={} '.format(epoch + 1, tmx, round(logs['acc'], 4),
...
# debug mode : use only a few data. and plot.
bDebug = False
...
if runmode == RUNMODE_TRAIN_ONE or runmode == RUNMODE_TRAIN_ALL:
    ...
    for modelname, (tri, tei) in zip(modellist, skf.split(nptrain, dftrain['label'].values)):
        ...
        hist = model.fit_generator(train_generator, initial_epoch=0, epochs=epochs, validation_data=val_generator,
                                   callbacks=cblist, steps_per_epoch=len(tri) / batch_size,
                                   validation_steps=len(tei) / batch_size)
