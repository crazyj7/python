...
'''
xception, 3count
image crop margin, resize
image black box noise add
f1score
stratifiedkfold. 5

fold;5. model 3counts.
image size ; 299.  batch;16. oom(32TT)
no use pseudo data.
'''
...
x_trainall = np.load(datadir+'x_train3.npy')
y_trainall = np.load(datadir+'y_train3.npy')
...
# modellist =  [ (21, 'xception'), (22, 'xception'), (23, 'xception')]
modellist = ['carmodel-v9-1-', 'carmodel-v9-2-', 'carmodel-v9-3-']

skf = StratifiedKFold(5, random_state=100)
...
#     modelpath = modelname+str(fold_c)+'-'+method+'-{epoch:03d}-{val_new_score:.4f}.ckpt'
    ...
    if False:
        ...
        x_train, y_train = x_trainall[tri], y_trainall[tri]
        x_val, y_val = x_trainall[tei], y_trainall[tei]
