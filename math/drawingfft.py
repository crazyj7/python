...
def get_firstbiggeridx(v, limit):
    idx=np.argwhere(v>limit).squeeze()
    return idx[0]
...
extraidx=get_firstbiggeridx(newt, 1)
print('extraidx=', extraidx)
# print('newt(extra)=', newt[extraidx:])
...
# added extra line for gibbs remove.
pi+=1
plt.subplot(ph, pw, pi)
plt.plot(oldx, oldy)
plt.scatter(oldx[extraidx], oldy[extraidx], c='r')

# fft image. (remove gibbs)
pi+=1
plt.subplot(ph, pw, pi)
